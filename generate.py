...
def generate(generator, discriminator, init_step, loader, total_iter=600000, test = 0, save_path = './result'):
    import os
    ...
    for i in pbar:
        ...
        while test > 0:
            ...
            #     utils.save_image(
            #         images,
            #         f'./{save_path}/{str(test)}_{str(i + 1).zfill(6)}.png',
            #         nrow=10,
            #         normalize=True,
            #         range=(-1, 1))
            #     test = test-1
                with torch.no_grad():
                    images = g_running(torch.randn(1, input_code_size).to(device), step=step,
                                       alpha=alpha).data.cpu()

                    utils.save_image(
                        images,
                        f'./{save_path}/{str(test)}.png',
                        nrow=1,
                        normalize=True,
                        range=(-1, 1))
                    test = test - 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_G', type=str, default="load G", help='error load G')
    parser.add_argument('--out_num', type=int, default=10, help='generate num')
    parser.add_argument('--trial_name', type=str, default="test1", help='a brief description of the training trial')
    parser.add_argument('--save_path', type=str, default="result", help='save_path')
    ...
    parser.add_argument('--init_step', type=int, default=6,
    ...
    g_running.load_state_dict(torch.load(args.load_G))
    # discriminator.load_state_dict(torch.load('./trial_experiment-1_2022-01-03_11_36/checkpoint/200000_d.model'))
    ...
    generate(generator, discriminator, args.init_step, loader, args.total_iter, args.out_num, args.save_path)# 10表示要保存多少张图片 后一个是保存路径
